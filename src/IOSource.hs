...
--   parse the "commented" text (see comment definition at
--   bottom of "Parse.hs") so we "invert" comments by adding
--   "\\vend" and "\\vbeg" to the ends of the document.
...
--   source position (as represented by the "SourcePos" type
--   from the "Parsec" library).
...
----------------------------------------------------------------
-- | Concrete syntax of source for expressions, with
--   accommodation for tags and annotations that can correspond
--   to errors or verification results.
...
----------------------------------------------------------------
-- | Collects state report from the results, if there is one.
